...
export default function EstadoChuva () {
  const [dados, setDados] = useState({ temperatura: '', chuva: '' });
  const [loading, setLoading] = useState(false);

  const buscarDados = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${enderecoServidor}/api/EstadoChuva`);
  ...
  return (
    <div className="max-w-md mx-auto bg-gradient-to-br from-blue-50 to-indigo-100 rounded-2xl shadow-xl p-6 border border-gray-200">
      <div>
            ...
            <div className="bg-white rounded-xl p-4 shadow-md border-l-4 border-red-400 hover:shadow-lg transition-shadow duration-300">
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-700"> Chuva</h3>
        ...
        <a href="https://wokwi.com/projects/441276477644798977" className="bg-blue-400 mt-4 p-3 text-white w-50 flex text-center rounded xl-8">Acessar WokWi</a>
    </div>
  );
}